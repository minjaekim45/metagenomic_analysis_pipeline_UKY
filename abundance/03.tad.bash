...
if [[ "$1" == "" || "$1" == "-h" ]] ; then
   ...
   folder       Path to the folder containing merged FASTA file (01.hq-set.fna).
...
grep "^>" 01.hq-set.fna | perl -pe 's/:.*//' | perl -pe 's/^>//' \
...
for i in $dir/29.TAD80/map/*.bam ; do
  ...
  echo -ne "$b\t"
  # Launch job
   sbatch --export="$OPTS" -J "TAD_$b" --account=$QUEUE --partition=$QOS --error "$dir"/zz.TMP/"TAD-$b"-%j.err -o "$dir"/zz.TMP/"TAD-$b"-%j.out  $pac/03.tad.pbs | grep .
