...
for fname in *.filtered_1.fq; do
  name=$(basename $fname .filtered_1.fq);
  newname="${name}.1_filtered.fq"
...
# Iterate over each file in the specified directory
for fname in *.filtered_2.fq; do
  name=$(basename $fname .filtered_2.fq);
  newname="${name}.2_filtered.fq"
  if [ "$fname" != "$newname" ]; then
    echo mv "$fname" "$newname"
    mv "$fname" "$newname"
  fi
