...
#SBATCH --job-name=IR37_binning      # Job name
...
for i in $dir/04.trimmed_fasta/*.CoupledReads.fa ; do
   ...
   sbatch --export="$OPTS" -J "Metabat2-$b" --account=$QUEUE --partition=$QOS --error "$dir"/zz.out/"Metabat2-$b"-%j.err -o "$dir"/zz.out/"Metabat2-$b"-%j.out  $cwd/run_Metabat2.pbs | grep .;
done 
